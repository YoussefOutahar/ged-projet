...
@Component(CourrielCreationTypes.COURRIEL_COMPLEMENT_CREATION_WITH_FILES)
public class CourrielComplementCreationWithFiles implements CourrielCreationInterface{
    ...
    public CourrielBo createCourriel(Long id, CourrielBo courrielBo, List<MultipartFile> files) {
        ...
        courrielPere.setComplement(courrielBo);
        CourrielBo afterSaveCourriel = courrielsRepository.save(courrielPere);

        courrielCreationUtils.handleNumeroCourriel(courrielBo);

        courrielCreationUtils.handleCourrielHistory(courrielBo, courrielPere,"Courriel Créé","Courriel Complement Ajouter");
