...
@Value("${uploads.location.directory}")
 ...
 @Value("${uploads.location.temp}")
    ...
    public ResponseEntity<DTO> update(DTO dto) throws Exception {
        ...
        if (dto.getId() == null || service.findById(dto.getId()) == null)
            ...
            if(updated != null){
                ...
                res = new ResponseEntity<>(HttpStatus.IM_USED);
            }
    ...
    public ResponseEntity<List<DTO>> delete(List<DTO> dtos) throws Exception {
        ...
        List<DTO> deletedAsDtos = new ArrayList<>();
        HttpStatus status = HttpStatus.CONFLICT;
        if (dtos != null && !dtos.isEmpty()) {
            converter.init(false);
            List<T> ts = converter.toItem(dtos);
            List<T> deleted = service.delete(ts);
            deletedAsDtos = converter.toDto(deleted);
            if (deleted.isEmpty()) {
        ...
        res = new ResponseEntity<>(deletedAsDtos, status);
        return res;
    }
