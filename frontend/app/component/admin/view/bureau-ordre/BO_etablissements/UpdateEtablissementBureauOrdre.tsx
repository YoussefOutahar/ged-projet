...
type Props = {
    t: TFunction;
    selectedEtablissement: EtablissementBureauOrdre;
...
const UpdateEtablissementBureauOrdre = ({ showToast, disabled, refetchEtablissements, selectedEtablissement, t, etablissements }: Props) => {
    ...
    useEffect(() => {
        if (selectedEtablissement) {
            formik.setValues(selectedEtablissement);
            setSelectedPays(allCountries.find((countryData) => countryData[0] === selectedEtablissement.pays) || null);
            formik.setFieldValue('pays', selectedEtablissement.pays);
        }
    }, [showUpdateDialog, selectedEtablissement]);


    const formik = useFormik({
        initialValues: new EtablissementBureauOrdre(),
        validationSchema: () => etablissementValidationSchema(etablissements.filter(e => e.id !== selectedEtablissement.id)),
        onSubmit: async (values) => {
            return await axios.put(`${ETABLISSEMENT_URL}${values.id}`, values).then((res) => {
    ...
    return (
        <>
            <Button raised disabled={disabled} label={t("modifier")} icon="pi pi-pencil" severity="secondary" className=" mr-2" onClick={() => setShowUpdateDialog(true)} />
            <Dialog
                ...
                footer={
                    <div className="p-dialog-footer">
                        <Button label={t('cancel')} icon="pi pi-times" onClick={() => setShowUpdateDialog(false)} />
...
export default UpdateEtablissementBureauOrdre