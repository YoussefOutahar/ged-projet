...
export default class DWTController extends React.Component {
    ...
    if (config) {
      if (config.prop === "Video Setup" || config.prop === "Camera Setup") {
        ...
        if (config.prop === "Video Setup") {
          bCamera = false;
          basicSetting = this.DWObject.Addon.Webcam.GetVideoPropertySetting(
            this.Dynamsoft.DWT.EnumDWT_VideoProperty["VP_" + config.value]
          );
          moreSetting = this.DWObject.Addon.Webcam.GetVideoPropertyMoreSetting(
            this.Dynamsoft.DWT.EnumDWT_VideoProperty["VP_" + config.value]
          );
        } else {
          basicSetting =
            this.DWObject.Addon.Webcam.GetCameraControlPropertySetting(
              this.Dynamsoft.DWT.EnumDWT_CameraControlProperty[
                "CCP_" + config.value
              ]
            );
          moreSetting =
            this.DWObject.Addon.Webcam.GetCameraControlPropertyMoreSetting(
              this.Dynamsoft.DWT.EnumDWT_CameraControlProperty[
                "CCP_" + config.value
              ]
            );
        ...
        switch (config.prop) {
          case "Frame Rate":
            this.DWObject.Addon.Webcam.SetFrameRate(config.value);
            break;
          case "Media Type":
            this.DWObject.Addon.Webcam.SetMediaType(config.value);
            break;
          case "Resolution":
            this.DWObject.Addon.Webcam.SetResolution(config.value);
            break;
          default:
            break;
        }
          ...
          if (this.toast.current) {
           // MessageService.showError(showToast, "Error!", "Entité déjà existante : Veuillez vérifier les données que vous avez fournies.");
            ...
            this.toast.current.show({
              severity: 'error',
              summary: 'Error: ',
              detail: 'Extension de fichier non reconnue, les extensions acceptées sont : pdf, jpg, png, tif, bmp' || errorString,
              life: 5000, // Set the display duration of the toast in milliseconds
            });
          }
          return;
        }
  ...
  saveOrUploadImage(_type) {
    ...
    let onConvertToBlobSuccess = (data) => {
      this.props.setUploadedFile(data);
    };

    let onConvertToBlobError = (error) => {
      console.log("onConvertToBlobError", error);
    };
        ...
        if (_type === "local") {
          switch (this.state.saveFileFormat) {
            ...
            case "tif":
              ...
              break;
            case "pdf":
              this.DWObject.SaveAllAsPDF(fileName, onSuccess, onFailure);
              break;
          }
        } else {
    ...
    if (_type === "server") {
      if (this.state.bUseFileUploader) {
        ...
        this.DWObject.GenerateURLForUploadData(
          imagesToUpload,
          fileType,
        ...
        this.DWObject.ConvertToBlob(
          imagesToUpload,
          fileType,
          onConvertToBlobSuccess,
          onConvertToBlobError
        );
  ...
  readBarcode() {
    ...
    this.DWObject.Addon.BarcodeReader.getRuntimeSettings().then((settings) => {
  ...
  doReadBarode(settings, callback) {
    ...
    this.DWObject.Addon.BarcodeReader.updateRuntimeSettings(settings).then(
      (settings) => {
  ...
  downloadOCRBasic(bDownloadDLL, _features) {
    ...
    let strOCRLangPath =
      this.Dynamsoft.DWT.ResourcesPath + "/addon/OCRBasicLanguages/English.zip";
    if (bDownloadDLL) {
      if (this.DWObject.Addon.OCR.IsModuleInstalled()) {
  ...
  ocr() {
    ...
    this.DWObject.Addon.OCR.SetOutputFormat(
      this.Dynamsoft.DWT.EnumDWT_OCROutputFormat.OCROF_TEXT
    );
    if (this.props.zones.length > 0) {
      ...
      this.DWObject.Addon.OCR.Recognize(
        ...
        (imageId, result) => {
          ...
          this.props.handleOutPutMessage(
            this.Dynamsoft.Lib.base64.decode(result.Get()),
            "info",
            false,
            true
          );
    ...
    let doRectOCR = (_zone, _zoneId) => {
      this.DWObject.Addon.OCR.RecognizeRect(
        ...
        (imageId, left, top, right, bottom, result) => {
          ...
          if (_zoneId === 0) this.props.handleOutPutMessage("", "", true);
          this.props.handleOutPutMessage(
            "OCR result in the rect [" +
              left +
              ", " +
              top +
              ", " +
  ...
  handleRangeChange(event) {
    ...
    if (value === "reset-range") {
      ...
      _type === "camera"
        ? this.DWObject.Addon.Webcam.SetCameraControlPropertySetting(
            this.Dynamsoft.DWT.EnumDWT_CameraControlProperty["CCP_" + prop],
            _default,
            true
          )
        : this.DWObject.Addon.Webcam.SetVideoPropertySetting(
                      ...
                      <ul>
                        ...
                        <li>
                          <select
                            ...
                            <option value="0">{t('document.b&w')}</option>
                            <option value="1">{t('document.gray')}</option>
                            <option value="2">{t('document.color')}</option>
                          </select>
                          <select
                            tabIndex="1"
                            style={{ width: "48%" }}
                  ...
                  <ul>
                    <li>
                      <label className="fullWidth">
                        <span style={{ width: "100%", fontWeight: "600"}}>{t('document.documentName')}</span>
                        <input
                          tabIndex="4"
                          style={{ width: "100%"}}
                          type="text"
                    ...
                    {/* <li>
                      ...
                      this.props.features & 0b10000000 ? (
                        <label>
                          <input
                            tabIndex="4"
                            title="Use Uploader"
                    ...
                    <li className="tc">
                      ...
                      {this.props.features & 0b10000 ? (
                        <button
                          ...
                          className={
                            this.props.buffer.count === 0
                              ? "majorButton disabled"
                              : "majorButton enabled width_4p"
                          }
                          disabled={
                            this.props.buffer.count === 0 ? "disabled" : ""
                          }
                    ...
                    <li className="tc">
                      ...
                      {this.props.features & 0b1000000 ? (
                        <button
                          ...
                          disabled={
                            this.props.buffer.count === 0 || this.state.ocring
                              ? "disabled"
                              : ""
                          }
                          onClick={() => this.ocr()}>
                          {this.state.ocring ? "Ocring..." : "OCR (English)"}
                        </button>
                      ) : (
                        ""
                      )}
